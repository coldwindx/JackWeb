...
const upload = async (files: File[]) => {
    ...
    // 查询已上传分片
    api.post('/api/slice/query', { condition: { hash: hash.value }, limit: 0 })
        .then((slices) => filterUploadedSlice(slices))
...
const filterUploadedSlice = (uploadedSlices: FileSlice[]) => {
    let uploadSliceIndexs = uploadedSlices.map((item: { chunkIndex: any; }) => item.chunkIndex);
    // 当前文件大小
...
const uploadSlice = (slices: FileSlice[]) => {    
    return Promise.all(slices.map((s) => {
        ...
        return api.post("/api/slice/upload", formData, headers)
